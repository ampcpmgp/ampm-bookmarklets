...
${bookmarklets.map(bm => `    <div class="bookmarklet-card">
      <h3 class="bookmarklet-title">${escapeHtml(bm.title)}${bm.version ? `<span class="version-badge">${escapeHtml(bm.version)}${bm.lastModified && !/\(\d{4}-\d{2}-\d{2}\)/.test(bm.version) ? ` (${escapeHtml(bm.lastModified)})` : ''}</span>` : ''}</h3>
